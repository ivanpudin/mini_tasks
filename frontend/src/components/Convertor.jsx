...
const Convertor = () => {
  ...
  const getValues = () => {
    if (convertResult.unit1 === 'kg' || convertResult.unit1 === 'lb' || convertResult.unit1 === 'grams') {
      setType([
        { short: 'kg', full: 'Kilograms' },
        { short: 'grams', full: 'Grams' },
        { short: 'lb', full: 'Pounds' },
      ])
    } else if (convertResult.unit1 === 'kph' || convertResult.unit1 === 'mps' || convertResult.unit1 === 'knots') {
      setType([
        { short: 'kph', full: 'Kilometers per hour' },
        { short: 'mps', full: 'Meters per second' },
        { short: 'knots', full: 'Knots' },
      ]) 
    } else if (convertResult.unit1 === 'kelvin' || convertResult.unit1 === 'celsius' || convertResult.unit1 === 'fahrenheit') {
      setType([
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      const res = await convertor(convertResult.unit1, convertResult.unit2, convertResult.quantity)
